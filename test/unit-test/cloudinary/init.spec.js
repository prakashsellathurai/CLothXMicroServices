...
describe('cloudinary client', () => {
  ...
  it('should not throw on empty Object', async function () {
    let sv = await save.product({}, 'testFolder')

    expect(() => sv).to.not.throw()
  })
  it('should not throw on nested Object', async function () {
    let sv = await save.product(undefined, 'testFolder')
    expect(() => sv).to.not.throw()
  })
